...
//
// This example displays a Google Map centered on New York City with a set of randomly placed
// animated markers. It includes three demos showing different ways to overlay content:
// - a basic animated map,
// - a draggable bottom sheet,
// - and a carousel slider overlay with images.
//
// Features:
// - Random marker generation around fixed static locations
// - Support for both PNG and SVG marker assets
// - Smooth scale animations on markers using the `AnimatedMapMarkersWidget`
// - Optional overlays like a draggable sheet or swipeable image carousel
//
// To run this example:
// - Ensure you have a valid Google Maps API key set up in your Android and iOS project configuration.
//
// Dependencies:
// - `animate_map_markers`: for animated marker handling and overlays
// - `google_maps_flutter`: for rendering Google Maps
// - `flutter_svg` (if SVG markers are used)
...
/// Generates a list of MarkerIconInfo using predefined locations and random marker assets
...
/// carousel slider overlay using remote images.
///
/// This demo demonstrates how to pair animated map markers with a
/// horizontally scrollable [MarkerSwipeCardConfig], which could be
/// used for photo galleries, event cards, or location details.
/// The slider supports image previews and page change callbacks.
...
/// - Reuses shared `staticLocations` list
///
/// Tip:
/// - You can customize the marker icon’s shadow, size, and color like any widget.
/// - Use `minMarkerSize` to help size the clickable area.
...
/// the assets folder (`assets/svg_map_marker.svg` is one example).
///
/// Requirements:
/// - Make sure to add `flutter_svg` package to your pubspec.yaml.
/// - Declare SVG assets in your pubspec.yaml under `assets:`
///
/// Example:
/// ```yaml
/// dependencies:
